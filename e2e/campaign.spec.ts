...
test.describe("campaign brain", () => {
  ...
  test("a sample advertiser streams a full plan into the panels", async ({ page }) => {
    ...
    await expect(publishers.getByTestId("publisher-card").first()).toContainText("Recommend");
    await expect(publishers).toContainText(/Excluded \(\d+\)/);

    await expect(page.getByTestId("personas")).toContainText("The Pet Parent");
    const creatives = page.getByTestId("creatives").getByTestId("creative-card");
    ...
    await expect(stepper.locator('[data-stage="config"]')).toHaveAttribute("data-state", "done");
  });

  test("junk input stops with questions instead of a fabricated plan", async ({ page }) => {
    await page.goto("/");
