...
export function PersonaPanel({ personas }: { personas: PersonaSelection | null }) {
  if (!personas) return <PanelSkeleton title="Personas" rows={2} />;
  return (
    <Card className="gap-0 py-4" data-testid="personas">
      <CardContent className="px-4">
        <div className="flex items-baseline gap-2">
          <div className="text-[15px] font-semibold">Personas</div>
          <span className="text-[12px] text-soft">
            {personas.selected.length} selected · {personas.rejected.length} rejected
          </span>
        </div>

        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {personas.selected.map((p) => (
            <div key={p.persona_id} className="rounded-xl border border-line p-3" data-testid="persona-card">
              <div className="flex items-center gap-2">
                <span className="text-[13.5px] font-semibold">{p.persona_name}</span>
                <ScoreBar value={p.fit_score} className="ml-auto" />
              </div>
              <p className="mt-1.5 text-[12px]">{p.why_plausible}</p>
              <p className="mt-2 rounded-lg bg-lilac px-2.5 py-1.5 text-[12px]">
                <span className="font-semibold text-accent-foreground">Angle · </span>
                {p.angle}
              </p>
              {p.watchouts.length > 0 && (
                <p className="mt-1.5 text-[12px] text-warn">Avoid: {p.watchouts.join(" · ")}</p>
              )}
              {p.best_publishers.length > 0 && (
                <p className="mt-1 font-mono text-[11px] text-soft">on {p.best_publishers.join(", ")}</p>
              )}
            </div>
          ))}
        </div>
