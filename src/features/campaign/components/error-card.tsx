...
const HINT: Record<string, string> = {
  rate_limit:
    "The model provider rate-limited the request. Wait a minute and retry.",
  timeout: "The model call timed out; a retry usually succeeds.",
  refusal: "The model declined this request.",
  validation:
    "The model's answer did not fit the expected shape twice in a row.",
...
export function ErrorCard({ error, onRetry }: Props) {
  const where =
    (STEP_TITLE as Record<string, string>)[error.stage as PipelineStage] ??
    error.stage;
  return (
    <div
    ...
    >
      ...
      {error.kind && HINT[error.kind] && (
        <p className="mt-1 text-[12px] text-soft">{HINT[error.kind]}</p>
      )}
