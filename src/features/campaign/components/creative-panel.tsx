...
export function CreativePanel({ creatives, progress, running }: Props) {
  const pending = running
    ? Math.max(0, (progress?.total ?? 3) - creatives.length)
    : 0;
  return (
    <DashCard
      id="creatives"
      data-testid="creatives"
      title="Creatives"
      subtitle={`${creatives.length} written${progress && running ? ` · ${progress.completed}/${progress.total}` : ""} · one ad unit per persona`}
    >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <Badge
          variant="outline"
          className={c.lint.passed ? "text-pass" : "text-fail"}
        >
          {c.lint.passed ? "lint ok" : "needs edit"}
          {c.lint.self_checks > 0
            ? ` · self-checked ×${c.lint.self_checks}`
            : ""}
        </Badge>
        <Button
          variant="ghost"
      ...
      <details className="mt-1.5">
        <summary className="cursor-pointer text-[12px] font-medium">
          Why this works for {c.persona_name}
        </summary>
        ...
        {c.target_publishers.length > 0 && (
          <p className="mt-1 font-mono text-[11px] text-soft">
            runs on {c.target_publishers.join(", ")}
          </p>
        )}
      </details>
      {c.lint.issues.length > 0 && (
        <ul className="mt-1.5 space-y-0.5 text-[11px]">
          {c.lint.issues.map((i) => (
            <li key={i.message} className="text-fail">
              {i.message}
            </li>
          ))}
        </ul>
