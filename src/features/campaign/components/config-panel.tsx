...
import type { CampaignConfig } from "@/types/api";

type Props = { config?: CampaignConfig };
...
export function ConfigPanel({ config }: Props) {
  if (!config)
    return <PanelSkeleton title="Campaign config" rows={2} id="config" />;
  ...
  return (
    <DashCard
      ...
      subtitle="the numbers a marketer reviews, then the JSON a system ingests"
      action={
        <div className="flex flex-wrap items-center gap-2">
          <Badge
            variant={notRecommended ? "destructive" : "default"}
            className="font-mono"
          >
            {config.status.replaceAll("_", " ")}
          </Badge>
          <Badge variant="outline">
            {config.objective.replaceAll("_", " ")}
          </Badge>
        ...
        <TabsContent value="plan">
          {config.publisher_allocation.length > 0 ? (
                ...
                <tbody>
                  {config.publisher_allocation.map((a) => (
                    ...
                    >
                      ...
                      <td className="py-1.5 pr-3 font-mono">
                        ${a.suggested_cpm_range_usd[0]}–
                        {a.suggested_cpm_range_usd[1]}
                      </td>
                      <td className="py-1.5 pr-3 font-mono">
                        {compact(a.est_impressions)}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <p className="mt-1 text-[13.5px] text-soft">
              No budget allocated: nothing was recommended.
            </p>
          )}

          <dl className="mt-3 grid gap-x-6 gap-y-1.5 text-[12px] sm:grid-cols-2">
            <Row label="Bid strategy">
              {config.bid_strategy.model} · start CPM{" "}
              {usd(config.bid_strategy.starting_cpm_usd)}
              {config.bid_strategy.target_cpa_usd !== null &&
                ` · target CPA ${usd(config.bid_strategy.target_cpa_usd)}`}
              {config.bid_strategy.max_cpc_usd !== null &&
                ` · max CPC $${config.bid_strategy.max_cpc_usd}`}
              <div className="text-soft">{config.bid_strategy.rationale}</div>
            </Row>
            <Row label="Budget">
              {usd(config.budget.total_usd)} over {config.budget.flight_days}{" "}
              days · {usd(config.budget.daily_cap_usd)}/day · pacing{" "}
              {config.budget.pacing}
            </Row>
            <Row label="KPIs">
              primary {config.kpis.primary}
              {config.kpis.secondary.length > 0 &&
                ` · secondary ${config.kpis.secondary.join(", ")}`}
              <div className="font-mono text-soft">
                {Object.entries(config.kpis.targets)
                  .map(([k, v]) => `${k}=${v}`)
                  .join("  ")}
              </div>
            </Row>
            <Row label="Forecast">
              {compact(config.forecast.impressions)} impressions ·{" "}
              {compact(config.forecast.clicks)} clicks ·{" "}
              {config.forecast.conversions} conversions
              {config.forecast.cpa_usd !== null &&
                ` · CPA ${usd(config.forecast.cpa_usd)}`}
            </Row>
            <Row label="Hygiene">
              frequency cap {config.frequency_cap.impressions}/
              {config.frequency_cap.per_days}d · rotation{" "}
              {config.creative_rotation.mode.replaceAll("_", " ")} after{" "}
              {compact(config.creative_rotation.optimize_after_impressions)}
            </Row>
          </dl>
        </TabsContent>
        ...
        <TabsContent value="targeting">
          <dl className="mt-1 grid gap-y-1.5 text-[12px]">
            <Row label="Demographics">
              {config.targeting.demographics.age_range ?? "any age"} ·{" "}
              {config.targeting.demographics.gender_skew} ·{" "}
              {config.targeting.demographics.income_tiers.join(", ") ||
                "any income"}
            </Row>
            <Row label="Interests">
              {config.targeting.interests.join(", ") || "—"}
            </Row>
            <Row label="Contextual">
              {config.targeting.contextual.join(", ") || "—"}
            </Row>
            <Row label="Geo">{config.targeting.geo.join(", ") || "—"}</Row>
            <Row label="Personas">
              {config.targeting.persona_ids.join(", ") || "—"}
            </Row>
            <Row label="Exclusions">
              {config.targeting.exclusions.join(" · ") || "—"}
            </Row>
          </dl>
        </TabsContent>
...
function Row({
  ...
  return (
    <div>
      <dt className="text-[11px] font-semibold tracking-wide text-soft uppercase">
        {label}
      </dt>
